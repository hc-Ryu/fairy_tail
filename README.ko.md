...
```mermaid
flowchart LR
    subgraph Round1["🎯 라운드 1: SOLVER"]
        G1["🔵 Gemini<br/>솔루션 A"]
        O1["🟢 OpenAI<br/>솔루션 B"]
    end

    subgraph Round2["🔍 라운드 2: CRITIC"]
        G2["🔵 Gemini<br/>비평"]
        O2["🟢 OpenAI<br/>비평"]
    end

    subgraph Round3["⚖️ 라운드 3: DEFENSE"]
        Defense["🔵 변호인<br/>(Gemini)"]
        Judge["🟠 판사<br/>(Claude)"]
        Prosecutor["🟢 검사<br/>(OpenAI)"]
    end
...
> 🎬 데모 영상 준비 중입니다! 이 저장소에 스타를 눌러 알림을 받으세요.
...
- [ ] 네이티브 Claude Code 지원을 위한 MCP 서버 통합
- [ ] VS Code 확장
- [ ] 세션 기반 지식 베이스 (토론 히스토리 학습)
- [ ] 토론 흐름 시각화를 위한 웹 대시보드
- [ ] 추가 LLM 제공자 지원 (Llama, Mistral)

기능 제안은 [토론](https://github.com/quantsquirrel/claude-synod-debate/discussions)에서 해주세요!
